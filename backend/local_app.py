...
from .application.ui import BLUE, GREEN, RED, WHITE, LocalUI
from .config import load_config
from .display.desktop_display import DesktopDisplay
from .display.interface import DisplayConfig
...
async def run(config_path: Path | None, display_mode: str, touch_mode: str) -> None:
    ...
    await display.draw_rgb565(0, 0, display.width, display.height, ui.render_home())
    ...
    try:
        while True:
            if touch is None:
                ...
                if event:
                    color = GREEN if event.kind == "down" else BLUE if event.kind == "move" else RED
                    for point in event.points:
                        await display.draw_rgb565(max(0, point.x - 4), max(0, point.y - 4), 8, 8, color.to_bytes(2, "big") * 64)
    finally:
